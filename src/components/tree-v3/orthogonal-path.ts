/**
 * tree-v3 — построение SVG path через последовательность точек на прямых
 * линиях (каждая соседняя пара делит либо x, либо y), скругляя каждый
 * внутренний угол маленькой квадратичной кривой вместо острого 90°.
 *
 * Копия геометрии из tree-v2/orthogonal-path.ts (которая сама — копия из
 * боевого src/components/tree/orthogonal-path.ts) — не импортируется
 * оттуда напрямую, чтобы tree-v3 оставался полностью независимой реализацией
 * (§38: tree-v3 не зависит от tree-v2 кода), но сама форма линий читается
 * как один консистентный "семейный" визуальный язык проекта.
 */
