...
export function MobileFocusView({
  ...
  const parentIds = new Set(
    graph.edges
      .filter((e) => e.kind === "parent_child" && e.target === focusNode.id)
      .map((e) => e.source),
  );
  const childIds = new Set(
    graph.edges
      .filter((e) => e.kind === "parent_child" && e.source === focusNode.id)
      .map((e) => e.target),
  );
  const spouseIds = new Set(
    graph.edges
      .filter(
        (e) =>
          e.kind === "partnership" &&
          (e.source === focusNode.id || e.target === focusNode.id),
      )
  ...
  const siblingIds = new Set(
    graph.nodes
      .filter(
        (n) =>
          n.generation === focusNode.generation &&
          n.id !== focusNode.id &&
          !spouseIds.has(n.id),
      )
      .map((n) => n.id),
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <RelativeRow title="Родители" nodes={parents} familyId={familyId} />
      <RelativeRow title="Супруги" nodes={spouses} familyId={familyId} />
      <RelativeRow title="Дети" nodes={children} familyId={familyId} />
      <RelativeRow
