...
describe("partialDateFromFormData", () => {
  ...
  it("returns undefined when the year field is empty", () => {
    ...
    expect(
      partialDateFromFormData(formData({ birthYear: "" }), "birth"),
    ).toBeUndefined();
  });

  it("parses a year-only date", () => {
    expect(
      partialDateFromFormData(formData({ birthYear: "1924" }), "birth"),
    ).toEqual({
      year: 1924,
      month: null,
      day: null,
  ...
  it("parses a fully-specified exact date", () => {
    expect(
      partialDateFromFormData(
        formData({ birthYear: "1924", birthMonth: "5", birthDay: "12" }),
        "birth",
      ),
    ).toEqual({
      year: 1924,
  ...
  it("treats a year+month (no day) as exact", () => {
    expect(
      partialDateFromFormData(
        formData({ birthYear: "1924", birthMonth: "5" }),
        "birth",
      ),
    ).toEqual({
      year: 1924,
  ...
  it("reads the approximate checkbox", () => {
    expect(
      partialDateFromFormData(
        formData({ birthYear: "1924", birthApproximate: "on" }),
        "birth",
      ),
  ...
  it("works with an arbitrary prefix (e.g. event date/endDate)", () => {
    expect(
      partialDateFromFormData(formData({ endDateYear: "1945" }), "endDate"),
    ).toEqual({
      year: 1945,
      month: null,
      day: null,
