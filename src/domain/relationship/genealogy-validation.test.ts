...
function date(year: number): PartialDate {
  return { year, month: null, day: null, precision: "year_only", isApproximate: false };
}
...
function pc(parentId: string, childId: string): ParentChildRecord {
  return { id: `${parentId}-${childId}`, familyId: "family-1", parentId, childId, parentRole: "biological" };
}

function partner(person1Id: string, person2Id: string, id = `${person1Id}-${person2Id}`): PartnershipRecord {
  return {
    id,
    familyId: "family-1",
    person1Id,
...
describe("validateGenealogyGraph — structural errors", () => {
  it("reports no issues for a clean, unremarkable family", () => {
    const graph = buildGenealogyGraph(
      [person("mother", { birthDate: date(1950) }), person("father", { birthDate: date(1948) }), person("child", { birthDate: date(1975) })],
  ...
  it("detects a 2-node cycle (A is B's parent and B is A's parent)", () => {
    const graph = buildGenealogyGraph([person("a"), person("b")], [pc("a", "b"), pc("b", "a")], []);
  ...
  it("detects a longer cycle (A -> B -> C -> A)", () => {
    ...
    expect(cycleIssue?.personIds).toEqual(expect.arrayContaining(["a", "b", "c"]));
  });
  ...
  it("reports no cycle for a normal acyclic multi-generation tree", () => {
    ...
    expect(validateGenealogyGraph(graph).filter((i) => i.kind === "self_parent_cycle")).toEqual([]);
  });
  ...
  it("detects a duplicated parent_child edge", () => {
    ...
    expect(validateGenealogyGraph(graph)).toContainEqual(
      expect.objectContaining({ kind: "duplicate_parent_child", severity: "error" }),
    );
  });
  ...
  it("does NOT flag two partnership rows for the same pair as duplicates (divorce + remarriage)", () => {
    ...
    expect(validateGenealogyGraph(graph).filter((i) => i.kind === "duplicate_partnership")).toEqual([]);
  });
});
...
describe("validateGenealogyGraph — date plausibility warnings", () => {
  it("warns (not errors) when a child's birth year is not after the parent's", () => {
    const graph = buildGenealogyGraph(
      [person("parent", { birthDate: date(1990) }), person("child", { birthDate: date(1985) })],
      [pc("parent", "child")],
      [],
    );
    const issues = validateGenealogyGraph(graph);
    expect(issues).toContainEqual(
      expect.objectContaining({ kind: "child_older_than_parent", severity: "warning" }),
    );
  });

  it("warns when a parent was implausibly young at a child's birth", () => {
    const graph = buildGenealogyGraph(
      [person("parent", { birthDate: date(2000) }), person("child", { birthDate: date(2005) })],
      [pc("parent", "child")],
      [],
    );
    expect(validateGenealogyGraph(graph)).toContainEqual(
      expect.objectContaining({ kind: "parent_too_young", severity: "warning" }),
    );
  });

  it("warns when death year precedes birth year", () => {
    const graph = buildGenealogyGraph([person("a", { birthDate: date(1950), deathDate: date(1940) })], [], []);
    expect(validateGenealogyGraph(graph)).toContainEqual(
      expect.objectContaining({ kind: "death_before_birth", severity: "warning" }),
    );
  });
  ...
  it("does not warn for a plausible, well-spaced birth gap", () => {
    const graph = buildGenealogyGraph(
      [person("parent", { birthDate: date(1960) }), person("child", { birthDate: date(1990) })],
      [pc("parent", "child")],
      [],
    );
    expect(validateGenealogyGraph(graph)).toEqual([]);
