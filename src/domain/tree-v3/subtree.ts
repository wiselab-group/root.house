...
/** Минимальный горизонтальный зазор край-в-край между двумя НЕ связанными родством парами/семьями, случайно совпавшими на одном Y (§11 "между семьями расстояние должно быть в 2 раза больше, чем между супругами", product decision) — ровно 2×SPOUSE_GAP, целенаправленно, а не совпадение с текущим числовым значением SIBLING_GAP. Используется в resolveAdjacentAncestorCouples (collision.ts) как целевой зазор между двумя независимыми дедовскими парами. */
