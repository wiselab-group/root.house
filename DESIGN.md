...
(обводка карточки, линия коннектора — это тот порог, не текстовый) — **кроме
`--branch`, см. отступление ниже**:
...
:root {
  ...
  --branch: oklch(0.68 0.03 50);         /* светло-бежевый — линии/ствол дерева, ~2.78:1 (см. отступление) */
...
.dark {
  ...
  --branch: oklch(0.45 0.03 50);         /* приглушённый, тот же ~2.4:1 масштаб контраста */
}
```

**Временное отступление от порога ≥3:1 (`--branch`, с 2026-09-18):**
пользователь показал референсный скриншот с тёплым «family archive» деревом
(iOS-мокап, вертикальная цепочка Mother→Spouse→Son), где линии-коннекторы
светло-бежевые/почти сливаются с фоном, а не насыщенный тёмно-коричневый
«ствол». По прямому запросу `--branch` перекалиброван под этот референс
визуально (~2.78:1 light / ~2.4:1 dark — ниже порога UI-границ), с явным
пониманием, что сам контраст будет пересмотрен отдельным заходом позже — не
«чинить» это обратно к тёмному высококонтрастному значению без отдельного
