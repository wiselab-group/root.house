...
Тёплая «семейная» палитра реализована (`src/app/globals.css`) — три hue с
чётко разделёнными ролями, никогда не смешиваемые в одном элементе:
...
- **Терракота** (hue 45, `--primary`) — единственный цвет ДЕЙСТВИЯ во всём
  ...
  ИСКЛЮЧИТЕЛЬНО focus-person/Relationship Trace/keyboard-selected карточка
  (`isFocusOrTraced`/`isSelected` в `person-node-parts.tsx`). Никогда не
  используется как состояние покоя.
- **Приглушённый sage/оливковый** (hue 126, `--tree-accent`) — цвет
  ...
  откалиброван под референсный логотип «Rooted» (выцветший, приглушённый, НЕ
  насыщенный «лесной» зелёный — см. DESIGN.md § Color Tokens за историей
  калибровки). `--chart-1..5` больше не привязаны к дереву — неиспользуемый
  shadcn-скаффолдинг для будущих графиков/статистики, обычная shadcn-палитра,
  без смысловой связи с брендом.
- **Тёплый коричневый** (hue 55, `--branch`) — цвет линий-коннекторов/
  «ствола» дерева (`relationship-edge.tsx`, `union-child-edge.tsx`), всегда
  структурный фон, никогда не incidental UI-акцент.

Ключевое правило: sage = «это человек» (постоянно), терракота = «это то, на
что ты сейчас смотришь/что выбрал» (временное состояние) — их нельзя менять
...
3. **Дети всегда рядом с родителями** — якорь ряда детей всегда прямая
   ...
   ребёнку далёкое свободное место: сначала пытаемся поднять ВЕСЬ РЯД
   ПОКОЛЕНИЯ родителя — не только его собственную ветвь предков
   (`collectAncestorBranchIds`), а каждого, кто в данный момент делит с ним
   один Y (его сиблингов/кузенов на том же ряду тоже), и для каждого из них
   — их собственную ветвь предков выше (иначе поднятый сосед столкнётся со
   своими предками) — на N шагов по `GENERATION_GAP` (перебор 1, 2, 3 шага,
   вверх раньше вниз), пока не найдётся сдвиг, под которым и весь сдвигаемый
   набор, и новая позиция ребёнка прямо под НОВЫМ рядом родителя
   (`positionByPerson`, а НЕ `junctionByPartnership` — это T-коннектор,
   точка на полпути между рядом родителя и рядом детей, а не сам ряд;
   перепутать их — отдельный баг, пойманный на реальных данных: ветвь
   поднималась на 2 поколения вместо 1, потому что «свободно» проверялось
   не там) свободны — весь сдвиг проверяется на коллизии ДО коммита,
   атомарно (частичный сдвиг без проверки самого ребёнка — ещё один баг,
   пойманный на реальных данных при первой версии этого прохода). Поднимать
   ТОЛЬКО пару родителя (не весь их ряд) — тоже был реальный баг, пойманный
   на реальных данных: пара отрывалась от своих ровесников (которые
   оставались на старом Y) и визуально «проваливалась» на чужой ряд —
   ряд поколения родителей ЭТИХ ровесников, читаясь как «на каком
   поколении эта пара?». Поднятие всего ряда целиком держит ряды поколений
   внутренне согласованными — просто ряд как единое целое сдвинут. Это
   **не** привязка к конкретному дискретному поколению — просто больший
   ...
   жёсткая привязка к ряду поколения» — единственное отступление в том, что
   после такого сдвига потомок (childId) сам оказывается на отдельном,
   промежуточном Y, не совпадающем с остальными его же ровесниками того же
   generation — осознанный, наименьший возможный побочный эффект: лучше
   один ребёнок на отдельном Y под своими (правильно согласованными)
   родителями, чем целый ряд поколения расколотый на два Y. Только если
   раскрутить ни ветвь, ни ряд предков не удаётся вовсе (например, у
   родителя нет собственных предков в графе) — fallback на
   `tryMoveChildNearParent`: 2D-поиск (X и Y) свободного слота рядом с
   junction, не требующий точного совпадения x. Триггер этого репейра —
   `findFirstFarFromParentViolator` — **не числовой порог «насколько далеко
   уже considered too far»**, а floating-point epsilon
   (`FAR_FROM_PARENT_X_EPSILON`, `subtree.ts`): любое отклонение одинокого
   листа от точного junction.x запускает репейр, без исключения. Числовой
   порог (изначально `CARD_WIDTH * 4`) был реальным багом, пойманным на
   реальных данных ПОСЛЕ первой версии этого правила: Наталья Свидунович
   (family Купчик/Ушкар, фокус Виктор Купчик) смещалась на 389px — заметно
   меньше порога в 704px — и репейр её молча пропускал, хотя она явно стояла
   не под родителями. Урок: «насколько далеко — это уже проблема» не может
   быть магическим числом, когда правило — «ребёнок ВСЕГДА строго под
   родителем» без исключений. См. память `tree-layout-downward-strand-gap`
   за историей находки и ревизий фикса на реальных данных (family Ушкар/
   Свидунович) — там же зафиксирован главный урок: сравнивать с `main` по
   фактическим числам, а не полагаться на «нет коллизий» как критерий
   готовности.
...
5. **Родные сиблинги рядом** — `growSiblingRow`/`growChildrenRowDown`
   ...
   порядке `childrenIds`, как непрерывный блок. Внутри `growSiblingRow`
   кровный родственник (сам сиблинг) всегда со стороны, обращённой к
   якорю ряда (ближе к остальным кровным родственникам), супруг —
   всегда с дальней стороны — независимо от пола (`personIsLeftOverride`
   в `growPersonBranchDown`/`ownCardOffsetFromAnchor`/
   `predictRowCardPositions`, явно переопределяет `shouldBeLeft`'s чисто
   гендерное правило именно в этом месте). Раньше здесь применялось
   голое гендерное правило независимо от родства — супруг мог оказаться
   между якорем и кровным родственником («spouseTowardAnchor», удалено)
   — исправлено на реальных данных.
