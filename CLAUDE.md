...
- **Терракота** (hue 45, `--primary`/`--ring`) — единственный цвет ДЕЙСТВИЯ во
  всём приложении, включая дерево: кнопки, badges, brand mark, а внутри
  дерева — Relationship Trace карточка (`isTraced` в
  ...
  Никогда не используется как состояние покоя. С 2026-09-13 `isSelected`
...
и линия, keyboard-selected карточка — обе теперь одной и той же double-ring
формы) — их нельзя менять местами или смешивать в одном элементе (см.
`buildCardFrameClassName`'s собственный doc-комментарий). Все три hue подобраны и проверены на
...
режим редактирования и фиксирует правки (`PhotoArrangeBar`, «Готово» режима
отметки людей в `photo-lightbox.tsx`), с иконкой галочки. Решение пользователя 2026-09-26: «готово» = «устоялось», а не новое
действие. Не распространять на обычные кнопки.
...
5. **Родные сиблинги рядом** — `growSiblingRow`/`growChildrenRowDown`
   ...
   — исправлено на реальных данных. **Ряд держится компактным независимо
   от глубины поддерева одного сиблинга** — `growChildrenRowDown`
   расставляет курсор между сиблингами по `SubtreeMeasurement.compactWidth`
   (собственная ширина ряда — карточка/партнёрство/remarriage side-by-side,
   БЕЗ descendant-глубины), никогда по `totalWidth` (который складывает всю
   ширину поддерева вниз). Реальный баг, найденный на синтетическом
   Adam→Cain/Abel/Seth фикстуре (Сиф — родоначальник 12-поколенной линии,
   ветвящейся у Ноя на 3 сыновей): до `compactWidth` три родных брата
   растягивались на тысячи px (Сиф уезжал на 3472px от Авеля), потому что
   курсор ряда учитывал всю глубину поддерева Сифа заранее. `compactWidth`
   есть и у `growPersonBranchDown`'s собственного remarriage-курсора (между
   несколькими браками ОДНОГО человека) и у зеркальных ему
   `predictRowCardPositions`/`ownCardOffsetFromAnchor` — если этот курсор
   использует `totalWidth`, а remarriage-партнёрство уже имеет своих детей,
   вторая свадебная ветка съезжает с компактной позиции, которую ожидал
   внешний ряд сиблингов, и накладывается на соседнего сиблинга (реальный
   баг, пойманный property-тестами: p-72×p-77 с overlapX=56). Правило:
   курсор МЕЖДУ единицами на одном ряду — всегда `compactWidth`; честная
   ширина каждой единицы (`totalWidth`) резервируется в occupancy только на
   ЕЁ СОБСТВЕННОМ Y и ниже, при рекурсии в её же детей — там, где широкое
   поддерево действительно раздвигается вбок обычным occupancy-поиском,
   никогда толкая сиблингов по родительскому ряду.
...
- **Multi-marriage** — при 2+ партнёрствах у ОДНОГО человека (внутри
  `growPersonBranchDown`, `subtree.ts`) сам человек всегда точно в позиции
  `anchorX` (не смещён, в отличие от единственного брака — там центр ПАРЫ
  на `anchorX`, сам человек смещён на пол-ширины пары), а его супруги
  чередуются по разные стороны от него по порядку брака —
  `multiPartnershipSpouseXs`: первый брак слева, второй справа, третий
  дальше слева и т.д. (`REMARRIAGE_GAP` между ветками одной стороны,
  `SPOUSE_GAP` между человеком и первой веткой каждой стороны) — реальный
  пример (Ламех между Адой и Циллой) явно запрошен пользователем. Solo-
  ветка (ребёнок без второго известного родителя, например Ной у Ламеха) не
  занимает боковую позицию, растёт прямо от `anchorX`/`ownX` вниз — и
  растится в `growPersonBranchDown` **раньше** детей обеих партнёрских пар
  (порядок кода: `if (solo) growChildrenRowDown(...)` до цикла по
  `partnerships`), чтобы гарантированно занять центральный слот, пока он
  ещё свободен: partnership-детские ряды растут НАРУЖУ от своего
  `branchCenter` (прочь от центра), так что если solo вырастает первым,
  дети обеих жён естественно объезжают уже занятую центральную колонку, а
  не наоборот (явный запрос пользователя: «жёны по краям — значит и их дети
  должны быть по краям», проверено на реальных данных: Ной ровно под
  Ламехом, дети Ады/Циллы раздвинуты по бокам). `measurePersonWidth`'s
  `compactWidth` для такого человека — `multiPartnershipRowWidth`
  (`subtree.ts`) — **симметрична** относительно `anchorX`
  (`CARD_WIDTH + 2*max(leftWidth, rightWidth)`), а не сумма реальных
  сторон: при нечётном числе браков (одна сторона на ветку больше другой)
  реальный юнит не центрирован на `anchorX`, но `growChildrenRowDown`'s
  курсор ряда сиблингов резервирует место, ЦЕНТРИРУЯ его на `childCenter`
  и ставя туда самого человека — несимметричный `compactWidth` резервировал
  верную ПЛОЩАДЬ, но не по факту центрированную область, и более широкая
  сторона могла наехать на соседнего сиблинга (реальный баг, пойманный
  property-тестами: человек с 3 браками, 2 ветки слева/1 справа).
  `growSiblingRow`'s (ряд дядей/тётей в предках) "near-edge card" — карточка
  юнита, обращённая к anchor'у ряда — при 2+ браках это НЕ сама карточка
  человека (он в центре), а крайняя ветка на стороне anchor'а —
  `nearEdgeOffsetFromAnchor` (`subtree.ts`) вычисляет это отдельно от
  `ownCardOffsetFromAnchor` (которая для 2+ браков всегда 0, так как сам
  человек всегда на `anchorX`). Хронологический порядок (`marriageOrder`) в
  `Partnership` пока не вычисляется реально (`normalizeGraph` всегда
  ставит `0` — есть в типе, не подключено) — порядок веток в alternating-
  последовательности берётся из порядка `person.partnershipIds`.
...
- **Развод = сплошная линия с генограммным `//` поверх неё** (изменено
  2026-09-13, заменяет более раннюю dasharray-схему). `PartnershipEdgeLine`
  (`relationship-edge.tsx`) всегда рисует партнёрскую линию ОДНИМ сплошным
  `path` — пунктирных партнёрских линий (ни текущего, ни бывшего брака)
  больше не существует, и линия НЕ разрезается на сегменты вокруг развода;
  развод (`isPastPartnership`, из `Partnership.isCurrent`) добавляет
  `DivorceBreakMark` — две короткие параллельные диагональные чёрточки
  (`DivorceSlash`), нарисованные ПОВЕРХ этой сплошной линии в её midpoint —
  классическая нотация генограмм (McGoldrick/Gerson), а не собственное
  изобретение. Заменило схему «частый пунктир `"5 3"` (брак) vs редкий
  `"2 4"` (развод)» — по прямому запросу пользователя после того, как обе
  разновидности пунктира на реальном скриншоте оказались визуально
  неразличимы («какой-то пунктир» и там, и там). Не переизобретать заново
  пунктирную схему.

  Реальный баг, пойманный сразу после первой версии на реальных данных:
  разведённая пара С ОБЩИМ РЕБЁНКОМ показывала полностью сплошную линию без
  видимого разрыва — `//` рисовался ровно в той же точке, что и
  `UnionCollapseBadge` (непрозрачная HTML-кнопка через `EdgeLabelRenderer`,
  поверх SVG-слоя рёбер), и кнопка эту метку полностью перекрывала. Первая
  попытка фикса — сдвинуть всю метку `//` целиком в сторону от бейджа —
  отклонена: несимметрично (метка прижата к одной из карточек) и визуально
  путано (три разных смысловых элемента — точка ветвления к ребёнку,
  collapse-кнопка, метка развода — толпились почти в одной точке). Финальное
  решение — `DivorceBreakMark`'s `straddle` (`relationship-edge.tsx`):
  вместо того чтобы двигать метку целиком, два диагональных штриха `//`
  РАЗЪЕДИНЯЮТСЯ — один ставится непосредственно перед бейджем, другой сразу
  после, каждый вне его ~20px «отпечатка» (`CollapseToggleButton`'s
  h-5/min-w-5) — так что видно ОДНУ метку `//`, внутри которой просто сидит
  кнопка, а не метку, отдельно сдвинутую куда-то вбок. Сам бейдж при этом не
  трогается — остаётся ровно на точке union-trunk'а (см. его собственный
  doc-комментарий). Когда collapse-бейджа на линии нет (`straddle`
  undefined), обе чёрточки по-прежнему рисуются рядом в midpoint, как единый
  `//`.

  ВАЖНО — уже пробовали и пользователь дважды явно ОТКЛОНИЛ физический
  разрыв самой линии (вырезать промежуток из `path`'s `d` двумя `M...L...`
  subpath'ами вместо одного сплошного сегмента, с горизонтальной линией
  между слешами скрытой): выглядело сломанным на скриншоте («ты опять
  сломал!»), хотя концептуально казалось «более настоящим» разрывом. Линия
  ДОЛЖНА оставаться одним нераздельным `path` — штрихи `//` рисуются только
  поверх неё. Не переделывать в эту сторону снова без явного нового запроса.
