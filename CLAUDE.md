...
3. **Дети всегда рядом с родителями** — якорь ряда детей всегда прямая
   ...
   под НОВЫМ рядом родителя (`positionByPerson`, а НЕ
   `junctionByPartnership` — это T-коннектор, точка на полпути между рядом
   родителя и рядом детей, а не сам ряд; перепутать их — отдельный баг,
   пойманный на реальных данных: ветвь поднималась на 2 поколения вместо
   1, потому что «свободно» проверялось не там) свободны — весь сдвиг
   проверяется на коллизии ДО коммита, атомарно (частичный сдвиг без
   проверки самого ребёнка — ещё один баг, пойманный на реальных данных
   при первой версии этого прохода). Это
