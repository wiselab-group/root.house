...
3. **Дети всегда рядом с родителями** — якорь ряда детей всегда прямая
   функция от `x` уже размещённого partnership. **Известный пробел**:
   единственный ребёнок без сиблингов/супруга иногда уезжает далеко от
   родителей, если его естественный ряд плотно занят чужой веткой
   (occupancy-поиск в `growChildrenRowDown` не ограничен по дальности) —
   коллизий и нарушений остальных инвариантов при этом нет, но связь
   родитель-ребёнок визуально не читается как «рядом». Подтверждено на
   реальных данных (не только синтетике). `repairSideConstraintViolations`
   эту конкретную форму бага не ловит (ловит только side-constraint и
   interleaved-sibling нарушения, не «далеко, но без коллизии») — не
   закрыто, следующий шаг записан в памяти
   (`tree-layout-downward-strand-gap`).
