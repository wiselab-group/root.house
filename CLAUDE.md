...
3. **Дети всегда рядом с родителями** — якорь ряда детей всегда прямая
   ...
   `findFirstFarFromParentViolator`/`tryMoveChildNearParent` (`subtree.ts`):
   для одинокого бездетного листа (без своего супруга/детей и без
   размещённых кровных сиблингов на ряду — не трогает случаи, где нужно
   двигать весь ряд целиком) ищет свободный 2D-слот (X И Y, не только Y)
   рядом с junction его родительского partnership, в радиусе до двух
   `GENERATION_GAP` — шире обычного `MAX_Y_NUDGE`, т.к. у листа нет
   потомков, чей интервал между поколениями мог бы сломаться от большего
   прыжка. См. память `tree-layout-downward-strand-gap` за историей находки
   на реальных данных (family Ушкар/Свидунович).
