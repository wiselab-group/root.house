...
- Файлы, загруженные, но не зарегистрированные (закрыли вкладку), удаляет
  ежедневный Vercel Cron `/api/cron/cleanup-uploads` — см. «Плановая очистка
  и `CRON_SECRET`» ниже.
...
### Плановая очистка и `CRON_SECRET`

**Зачем очистка.** Загрузка идёт в два шага: браузер кладёт файл прямо в
Blob, затем отдельным запросом регистрирует его в БД. Если между шагами
закрыть вкладку, пропадёт сеть или регистрация будет отклонена, файл
остаётся в хранилище без записи в `media`. Его никто не видит, но место он
занимает и оплачивается. Раз в сутки (03:00 UTC, `vercel.json` → `crons`)
Vercel вызывает `GET /api/cron/cleanup-uploads`. Эндпоинт удаляет такие
файлы (`domain/media/media-cleanup.ts`), но только:

- в папках прямой загрузки `<familyId>/uploads/` и `<familyId>/variants/`;
  старые оригиналы и прочие файлы хранилища не трогаются;
- старше суток: свежий файл законно не зарегистрирован несколько секунд
  между двумя шагами загрузки.

**Зачем `CRON_SECRET`.** Эндпоинт cron — обычный публичный URL: вызвать его
может кто угодно. Он удаляет данные, поэтому должен выполняться только по
вызову самого Vercel Cron. Для этого Vercel при каждом вызове cron-задачи
передаёт значение переменной окружения `CRON_SECRET` в заголовке
`Authorization: Bearer <CRON_SECRET>`. Роут сверяет заголовок с тем же
значением из своего окружения. Если значения не совпали или переменная не
задана, он отвечает `401` и ничего не делает. Отказ без секрета выбран
намеренно: забытая переменная выключает очистку, а не открывает её всем.

**Настройка:**

- Сгенерировать значение: `openssl rand -hex 32`.
- Задать его в Vercel: Project → Settings → Environment Variables →
  `CRON_SECRET`, окружение Production (cron-задачи Vercel запускает только на
  production-деплое). Переменная подхватывается со следующего деплоя.
- В `.env.local` переменная не нужна: локально cron не запускается. Ручной
  запуск того же кода — `pnpm media:cleanup-orphans` (`--dry-run` только
  выводит список); секрет ему не нужен, скрипт работает с БД и Blob напрямую.
- Секрет нигде не хранится в коде и в git: только в окружении Vercel. При
  утечке достаточно заменить значение и сделать новый деплой.
...
- **Visibility** (`db/schema/privacy.ts`) — что участник может видеть,
  ...
  **Известный пробел**: визуализация дерева (`/families/[slug]/tree`) пока
  ...
  genealogy-aware перекомпоновки, вне скоупа текущей итерации. Соседний
  прецедент существует (не закрывает пробел, чисто информационно): Stage 5
  переписывания layout-движка (`tree-layout-rewrite`) добавил чисто
  клиентское, эфемерное сворачивание веток потомков
  (`components/tree/prune-collapsed.ts` +
  `use-collapsed-branches.ts`) — обрезка уже готового, спозиционированного
  `TreeLayoutGraph` по дереву `parent_child`-рёбер, без повторного прогона
  layout-движка. Тот же паттерн (пред-layout или пост-layout обрезка графа
  по набору id) применим и к privacy-фильтрации PRIVATE-персон, но
  сознательно не был реализован в рамках этой задачи — сворачивание веток
  управляется самим пользователем и симметрично (каждый видит одно и то же
  дерево, просто сворачивает/разворачивает у себя), тогда как
  privacy-фильтрация асимметрична (разные участники видят разные графы) и
  требует более аккуратной genealogy-aware перекомпоновки линий-коннекторов
  вокруг скрытого узла, а не просто скрытия поддерева.
